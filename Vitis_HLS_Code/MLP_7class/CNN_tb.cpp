...
#include "CNN.h"
#include "Dense.h"
#define NumberOfPicture 2723
#define d 1
int main(){
	float OutModel;
	float* Weights = (float*)malloc(892 * sizeof(float));
	float tmp;
	...
	fclose(Weight);
	////read Input
	float* InModel = (float*)malloc((NumberOfPicture * d * 16) * sizeof(float));
	FILE* Input = fopen("x_test.txt", "r");
	for (int i = 0; i < NumberOfPicture * d * 16; i++){
		fscanf(Input, "%f", &tmp);
		*(InModel + i)=tmp;
	}
	fclose(Input);
