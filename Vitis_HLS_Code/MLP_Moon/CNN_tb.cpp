...
#include "CNN.h"
#include "Dense.h"
#define NumberOfPicture 3000
#define d 1
int main(){
	float OutModel;
	float* Weights = (float*)malloc(65 * sizeof(float));
	float tmp;
	...
	fclose(Weight);
	////read Input
	float* InModel = (float*)malloc((NumberOfPicture * d * 2) * sizeof(float));
	FILE* Input = fopen("X_test.txt", "r");
	for (int i = 0; i < NumberOfPicture * d * 2; i++){
		fscanf(Input, "%f", &tmp);
		*(InModel + i)=tmp;
	}
	fclose(Input);
